...
team.data$Team[team.data$ratio == min(team.data$ratio)]  # Boston Celtics*
...
BetterShooters <- function(team.one, team.two) {
  better.shooter.team <- filter(team.data, Team %in% c(team.one, team.two)) %>%
                         filter(FG. == max(FG.)) %>%
                         select(Team, FG., PTS)
  return(better.shooter.team)
}
